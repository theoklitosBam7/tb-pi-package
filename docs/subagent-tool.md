...
| `model`                | string  | Model as `provider/id`. Overrides the agent's frontmatter model.                   |
| `tasks`                | array   | Parallel mode. Each item accepts `agent`, `subagent_type`, `task`, `cwd`, `model`. |
...
The tool resolves each agent's model in this order: the tool input `model`, the settings override, the agent frontmatter, then the parent agent's model. Settings overrides are described in [Subagent overrides](subagent-overrides.md).
